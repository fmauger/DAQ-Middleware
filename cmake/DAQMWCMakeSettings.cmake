...
# ---------------
#
# Common and useful CMake settings for DAQMW C/C++ projects.
#
...
#
#   - DAQMW standard relative path for resources
#
...
# ^^^^^^^^^^^^^^^^^^^
#
# DAQMW project supports GNU and Clang compilers directly, with the following set of
# default flags:
#
...
# ^^^^^^^^^^^^^^^^^^^^^^
#
# DAQMW project require support for C++11 at minimum, but users may optionally
...
#-----------------------------------------------------------------------
# "Header guard"
if(__DAQMWCMAKESETTINGS_INCLUDED)
  return()
else()
  set(__DAQMWCMAKESETTINGS_INCLUDED 1)
endif()

#-----------------------------------------------------------------------
# Include needed modules
#
include(DAQMWCMakeUtilities)
...
foreach(_dir BINDIR LIBDIR INCLUDEDIR DATAROOTDIR LIBEXECDIR)
...
message(STATUS "CMAKE_INSTALL_FULL_CMAKEDIR=${CMAKE_INSTALL_FULL_CMAKEDIR}")
...
message(STATUS "PROJECT_BUILD_PREFIX=${PROJECT_BUILD_PREFIX}")
set(PROJECT_TAG "${PROJECT_NAME}-${PROJECT_VERSION}")
message(STATUS "PROJECT_TAG=${PROJECT_TAG}")
foreach(_path ITEMS BINDIR LIBDIR INCLUDEDIR DATAROOTDIR CMAKEDIR PLUGINDIR CMAKEMODULEDIR LIBEXECDIR)
  set(PROJECT_BUILD_${_path} "${PROJECT_BUILD_PREFIX}/${CMAKE_INSTALL_${_path}}")
  message(STATUS "PROJECT_BUILD_${_path}=${PROJECT_BUILD_${_path}}")
endforeach()
...
set(PROJECT_CMAKEMODULE_DIR   "${PROJECT_TAG}/cmake")
message(STATUS "PROJECT_RESOURCE_DIR=${PROJECT_RESOURCE_DIR}")
message(STATUS "PROJECT_EXAMPLE_DIR=${PROJECT_EXAMPLE_DIR}")
message(STATUS "PROJECT_DOCUMENTATION_DIR=${PROJECT_DOCUMENTATION_DIR}")
message(STATUS "PROJECT_CMAKEMODULE_DIR=${PROJECT_CMAKEMODULE_DIR}")
...
message(STATUS "PROJECT_BINDIR_TO_RESOURCEDIR=${PROJECT_BINDIR_TO_RESOURCEDIR}")
...
message(STATUS "PROJECT_LIBDIR_TO_PREFIX=${PROJECT_LIBDIR_TO_PREFIX}")
file(RELATIVE_PATH PROJECT_LIBDIR_TO_BINDIR      "${CMAKE_INSTALL_FULL_LIBDIR}" "${CMAKE_INSTALL_FULL_BINDIR}")
message(STATUS "PROJECT_LIBDIR_TO_BINDIR=${PROJECT_LIBDIR_TO_BINDIR}")
...
message(STATUS "PROJECT_LIBDIR_TO_RESOURCEDIR=${PROJECT_LIBDIR_TO_RESOURCEDIR}")
file(RELATIVE_PATH PROJECT_CMAKEDIR_TO_CMAKEMODULEDIR "${CMAKE_INSTALL_FULL_CMAKEDIR}/${PROJECT_TAG}"
  "${CMAKE_INSTALL_FULL_DATAROOTDIR}/${PROJECT_CMAKEMODULE_DIR}")
message(STATUS "PROJECT_CMAKEDIR_TO_CMAKEMODULEDIR=${PROJECT_CMAKEDIR_TO_CMAKEMODULEDIR}")
file(RELATIVE_PATH PROJECT_CMAKEDIR_TO_INCLUDEDIR     "${CMAKE_INSTALL_FULL_CMAKEDIR}/${PROJECT_TAG}" "${CMAKE_INSTALL_FULL_INCLUDEDIR}")
message(STATUS "PROJECT_CMAKEDIR_TO_INCLUDEDIR=${PROJECT_CMAKEDIR_TO_INCLUDEDIR}")
...
foreach(_varname 
    ...
    CMAKEDIR_TO_CMAKEMODULEDIR
    )
  set(${PROJECT_NAME_UC}_${_varname} "${PROJECT_${_varname}}")
  message(STATUS "${PROJECT_NAME_UC}_${_varname}=${${PROJECT_NAME_UC}_${_varname}}")
endforeach()
...
set(CMAKE_LIBEXEC_OUTPUT_DIRECTORY "${PROJECT_BUILD_LIBEXECDIR}")
set(CMAKE_INCLUDE_OUTPUT_DIRECTORY "${PROJECT_BUILD_INCLUDEDIR}")
set(CMAKE_DATAROOT_OUTPUT_DIRECTORY "${PROJECT_BUILD_DATAROOTDIR}")
message(STATUS "PROJECT_BUILD_LIBEXECDIR=${PROJECT_BUILD_LIBEXECDIR}")
message(STATUS "CMAKE_ARCHIVE_OUTPUT_DIRECTORY=${CMAKE_ARCHIVE_OUTPUT_DIRECTORY}")
message(STATUS "CMAKE_RUNTIME_OUTPUT_DIRECTORY=${CMAKE_RUNTIME_OUTPUT_DIRECTORY}")
message(STATUS "CMAKE_LIBRARY_OUTPUT_DIRECTORY=${CMAKE_LIBRARY_OUTPUT_DIRECTORY}")
message(STATUS "CMAKE_LIBEXEC_OUTPUT_DIRECTORY=${CMAKE_LIBEXEC_OUTPUT_DIRECTORY}")
message(STATUS "CMAKE_INCLUDE_OUTPUT_DIRECTORY=${CMAKE_INCLUDE_OUTPUT_DIRECTORY}")
message(STATUS "CMAKE_DATAROOT_OUTPUT_DIRECTORY=${CMAKE_DATAROOT_OUTPUT_DIRECTORY}")
...
# - Prepend DAQMW's flags to any preexisting ones, allowing user
